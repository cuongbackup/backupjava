...
public class Validation {
    ...
    public static int checkInputIntLimit(int min, int max) {
        while (true) {
            try {
                ...
                System.err.println("Input number between [" + min + ", " + max + "]");
    ...
    public static int checkInputGraduation(int birthDate) {
        while (true) {
            int yearGraduate = Integer.parseInt(sc.nextLine().trim());
            if (yearGraduate - birthDate < 21) {
                System.err.println("Must be greater than " + (birthDate + 21));
                System.out.print("Enter again: ");
            } else {
                return yearGraduate;
            }
